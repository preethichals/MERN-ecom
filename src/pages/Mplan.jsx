...
function Mplan() {
  ...
  const getAllPlans = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`https://ecommerce-rm2m.onrender.com/api/v1/plan/plan-list/${page}`);
      setPlan(data.plan);
      setLoading(false);
    } catch (error) {
      console.log(error);
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get("https://ecommerce-rm2m.onrender.com/api/v1/plan/plan-count");
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
    }
  ...
  const filterPlan = async () => {
    try {
      const { data } = await axios.post("https://ecommerce-rm2m.onrender.com/api/v1/plan/plan-filter", {
        checked,
      });
      setPlan(data?.plan);
    } catch (error) {
